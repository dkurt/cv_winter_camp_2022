...
void prewitt_x_parallel_vec_wrap(const Mat& src, Mat& dst) {
    ...
    parallel_for_(Range(0, src.rows), [&](const Range& range) {
        for (int y = range.start; y < range.end; ++y) {
            ...
            int x = 0;
            for (; x <= dst.cols - v_uint8::nlanes; x += v_uint8::nlanes) {
                v_uint8 res = v_add_wrap(v_sub_wrap(vx_load(psrc0 + x + 2),
                                                    vx_load(psrc0 + x)),
                              v_add_wrap(v_sub_wrap(vx_load(psrc1 + x + 2),
                                                    vx_load(psrc1 + x)),
                                         v_sub_wrap(vx_load(psrc2 + x + 2),
                                                    vx_load(psrc2 + x))));
                v_store(pdst + x, res);
            }
            for (; x < dst.cols; ++x) {
...
void prewitt_x_parallel_vec_wrap2(const Mat& src, Mat& dst) {
    ...
    parallel_for_(Range(0, src.rows), [&](const Range& range) {
        ...
        for (; y <= range.end - 2; ++y) {
            ...
            int x = 0;
            for (; x <= dst.cols - v_uint8::nlanes; x += v_uint8::nlanes) {
                v_uint8 res = v_add_wrap(v_sub_wrap(vx_load(psrc1 + x + 2),
                                                    vx_load(psrc1 + x)),
                                         v_sub_wrap(vx_load(psrc2 + x + 2),
                                                    vx_load(psrc2 + x)));
