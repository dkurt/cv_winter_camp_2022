...
void roberts_parallel_vec(const cv::Mat& src, cv::Mat& dst) {
    ...
    parallel_for_(Range(0, src.rows), [&](const Range& range) {
    for (int y = range.start; y < range.end; ++y) {
            const uint8_t* psrc0 = bsrc.ptr<uint8_t>(y);
            const uint8_t* psrc1 = bsrc.ptr<uint8_t>(y + 1);
            int16_t* pdst = dst.ptr<int16_t>(y);
            int x = 0;
            for (; x < dst.cols - v_uint16::nlanes; x += v_uint16::nlanes) {

                v_int16 resdx = v_reinterpret_as_s16(vx_load_expand(psrc0 + x)) - v_reinterpret_as_s16(vx_load_expand(psrc1 + x + 1));
                v_int16 resdy = v_reinterpret_as_s16(vx_load_expand(psrc0 + x + 1)) - v_reinterpret_as_s16(vx_load_expand(psrc1 + x));

                v_int16 res = resdx * resdx + resdy * resdy;
